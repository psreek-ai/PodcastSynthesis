...
from app.db.database import get_feed, init_db
...
def get_knowledge_history(limit: int = 50):
    """
    Returns the concepts stored in the user's Knowledge Graph.
    Inspired by PodcastKnowledgeDistiller's /api/knowledge/history endpoint —
    gives users visibility into what they've learned and when.
    """
    try:
        kg = KnowledgeGraph()
        ...
        peek = kg.collection.peek(limit=limit)
        ids = peek.get("ids", [])
        docs = peek.get("documents", [])
        metadatas = peek.get("metadatas", []) or [{}] * len(docs)

        concepts = [
            {
                "id": ids[i] if i < len(ids) else f"concept_{i}",
                "text": docs[i],
                "source": metadatas[i].get("source", "") if metadatas[i] else "",
                "task_id": metadatas[i].get("task_id", "") if metadatas[i] else "",
            }
            for i, doc in enumerate(docs)
            if doc
        ]

        return {"concepts": concepts, "total": count}

    except Exception as exc:
        logging.error(f"Knowledge history error: {exc}")
        return {"concepts": [], "total": 0}


@app.delete("/api/knowledge/{concept_id}")
def delete_knowledge_concept(concept_id: str):
    """
    Removes a specific concept from the Knowledge Graph by its ID.
    Mirrors the DELETE /api/knowledge/:id endpoint from PodcastKnowledgeDistiller.
    """
    try:
        kg = KnowledgeGraph()
        kg.collection.delete(ids=[concept_id])
        return {"status": "deleted", "id": concept_id}
    except Exception as exc:
        logging.error(f"Failed to delete concept {concept_id}: {exc}")
